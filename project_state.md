...
Animator (US-011) — PixiJS-рендерер с fallback-статусом при отсутствии WebGL

Экраны: Setup → [Countdown] → Session → Stats → Setup

V2 (планируется):
Animator → LocationRegistry → ForestLocation (PixiJS)
                            → [будущие локации]
```
...
1. US-012: Система локаций (архитектура)
2. US-013: Локация «Лес/Природа»
