...
GamePhrases — общий словарь реплик персонажа (positive/miss) и helper выбора без мгновенного повтора
SpeechService — общий overlay-рендерер реплик; режим выбирается метаданными локации в LocationRegistry (`speechMode`: overlay/in-canvas)
...
US-019: Полноэкранные edge-вспышки по зонам точности — завершён 2026-02-10
...
- Реплики: режим задаётся в LocationRegistry (`speechMode`): overlay по умолчанию, in-canvas для локаций вроде RunnerLocation
- Edge-вспышки в RunnerLocation имеют FPS-based degrade/recover (hysteresis), чтобы не перегружать слабый GPU (Windows 7 + Chrome)
