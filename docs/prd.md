...
## V2.1 — Следующая итерация (UI Runner)

### Главная цель
Сделать обратную связь в RunnerLocation более понятной и эмоциональной: ребёнок должен сразу видеть качество попадания в бит и получать мотивацию продолжать.

### Что делаем

1. **Упрощаем Runner-сцену** — убираем препятствия как основной элемент, чтобы визуал не конфликтовал с механикой ритма (прыжок/спотыкание по попаданию в бит).
2. **Добавляем streak/комбо** — серия успешных попаданий (`perfect`/`good`) с отдельной UI-индикацией и сбросом на `miss`.
3. **Добавляем реплики динозавра** — короткие фразы при прыжке и при спотыкании (поддержка фразовых наборов для вариативности).
4. **Усиливаем фидбек зон** — вместо слабых локальных волн делаем яркие цветовые edge-вспышки по краям экрана: зелёный (`perfect`), жёлтый (`good`), красный (`miss`).

### Не делаем в этой итерации

- Новые локации
- Новую игровую механику поверх ритма
- Изменение порогов зон точности
