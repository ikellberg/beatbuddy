...
Файл `project_state.md` в корне каждого проекта — слепок состояния для быстрого погружения LLM.

Обязательные разделы:
- **Architecture** — как устроена система на верхнем уровне (модули, связи, потоки данных)
- **Current Sprint** — что в работе прямо сейчас
- **Backlog** — приоритетный список (ID + название, без деталей)
- **Done** — компактно (диапазон или одна строка на этап)
- **Gotchas** — грабли и неочевидные решения, которых нет в коде

Правила:
- Обновляется после каждой завершённой задачи
- Не дублировать PRD, user stories, структуру файлов
